...
class WorkoutPlanFragment : Fragment(){
    ...
    override fun onCreateView(
        ...
        val config = SyncConfiguration.Builder(user!!, partition)
            .build()
        val core = userRealm.where<Plan_Core>().findFirst()
        val pull = userRealm.where<Plan_Pull>().findFirst()
        val push = userRealm.where<Plan_Push>().findFirst()

        Realm.getInstanceAsync(config, object: Realm.Callback() {
            override fun onSuccess(realm: Realm) {
                // since this realm should live exactly as long as this activity, assign the realm to a member variable
                this@WorkoutPlanFragment.exerciseRealm = realm
        ...
        Realm.getInstanceAsync(user_config, object: Realm.Callback() {
            override fun onSuccess(realm: Realm) {
                // since this realm should live exactly as long as this activity, assign the realm to a member variable
                this@WorkoutPlanFragment.userRealm = realm
            }
        })
    //change later to btnWP
        ...
        searchview.setOnQueryTextListener(object: SearchView.OnQueryTextListener{
            override fun onQueryTextSubmit(text: String): Boolean {
                recyclerSearch(exerciseRealm, user, partition, text)
                return false
            }
        ...
        //change values later for workout plan
