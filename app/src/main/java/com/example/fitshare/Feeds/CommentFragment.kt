    ...
    override fun onCreateView(
        ...
        Realm.getInstanceAsync(post_config, object: Realm.Callback() {
            override fun onSuccess(realm: Realm) {
                ...
                equalTo("_id", ObjectId(postID)).findFirst()
            }
        })
        ...
        Realm.getInstanceAsync(config, object: Realm.Callback() {
            override fun onSuccess(realm: Realm) {
                ...
                adapter = CommentAdapter(commentRealm.where<Comment>().sort("date").findAll(), user!!, partition)
        ...
        commentLayout.setEndIconOnClickListener{
            ...
            //Toast.makeText(activity, txtComment.text.toString(), Toast.LENGTH_SHORT).show()
            val comment = Comment(txtComment.text.toString(), user!!.id)

            postRealm.executeTransactionAsync { transactionRealm: Realm ->
                post?.comments?.add(comment)
                transactionRealm.insertOrUpdate(post)
            }
